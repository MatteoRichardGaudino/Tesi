...
Si definisce la funzione $arity : \Sigma_f \cup \Sigma_p \rightarrow \mathbb{N}$ che associa ad ogni simbolo di funzione o predicato la sua arietà.
Funzioni e predicati di arietà 0 sono detti rispettivamente \textit{Funzioni costanti} e \textit{Predicati costanti}.
...
\begin{itemize}
  ...
  \item Se $\tau_1, ..., \tau_n$ sono termini e $f$ è un simbolo di funzione di arietà $n$ allora $f(\tau_1, ..., \tau_n)$ è un termine
\end{itemize}
...
di arietà $n$ e $\tau_1, ..., \tau_n$ sono termini.
...
Due letterali della stessa arietà $L_1 = p_1(\tau_1, ..., \tau_n)$ e $L_2 = p_2(\tau_1', ..., \tau_n')$ 
sono unificabili se e solo se esiste una sostituzione che li eguaglia ignorando il simbolo di predicato.
In altre parole sia $f$ una funzione di arietà $n$, allora $L_1$ e $L_2$ sono unificabili se e solo se sono unificabili i termini
$f(\tau_1, ..., \tau_n)$ e $f(\tau_1', ..., \tau_n')$.
Letterali di diversa arietà non sono mai unificabili.
...
\begin{proposition}
  ...
  \begin{enumerate}
    \item I due termini hanno arietà diverse
...
\begin{algorithm}[H] \label{alg:unificazione}
  ...
  \While{S is not Empty}{
    ...
    \If{$\tau_1 \neq \tau_2$}{
      \Switch{$\tau_1 \tau_2$}{
        \Case{$\tau_1$ is a variable $x$ and $\tau_2$ is a variabile $y$}{$\sigma := \sigma \cup \{x/y\}$}
        \Case{$\tau_1$ is a variable $x$}{
          \If{x in $\tau_2^\sigma$}{
            \Return{$\bot$}
          } \Else {
        ...
        \Case{$\tau_1 = f(u_1, ..., u_n)$ and $\tau_2 = f(t_1, ..., t_n)$} {$S.push(u_1, t_1), ..., S.push(u_n, t_n)$}
        \Case{$\tau_1 = f(u_1, ..., u_n)$ and $\tau_2 = g(t_1, ..., t_m)$} {\Return{$\bot$}}
      }
    }
  }
  \Return{$\sigma$}\;
...
\begin{itemize}
  ...
  % \item Se $\tau$ è una costante $c$ allora $\tau^\mathfrak{I}_\gamma = c^\mathfrak{I}$ (Le costanti non dipendono dal contesto)
...
% ASSOLUTAMENTE NO Un caso d'uso tipico è quello di voler unificare due letterali di due clausole diverse. Con l'assunzione che le variabili siano tutte diverse è possibile
...
% Da aggiungere il riferimento ai frattali e ai libri e disclamer sul fatto che non è una trattazione esaustiva
...
capire il funzionamento di un sistema \textit{ATP} (Automated Theorem Prover), nonché 
...
% Si prenda ad esempio il gioco del \textit{MU} dal libro di Hofstadter.
...
% ed evidenzia la necessità di sviluppare sempre nuove tecniche per risolvere il problema della soddisfacibilità.
...
TPTP è una nota libreria di problemi utilizzata per testare e valutare diversi sistemi ATP.
