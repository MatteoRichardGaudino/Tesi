...
Un esempio di inferenza è la regola del \textit{Modus Ponens}. Data una formula $\varphi \land (\varphi \Rightarrow \psi)$ si può inferire $\psi$.
Viene spesso utilizzato il simbolo $P \vdash C$ per indicare l'applicazione di un'inferenza alla \textit{Premessa} $P$ per ottenere una \textit{Conclusione} $C$.
Si considera inferenza anche una funzione da $F^n$ in $F$ ma in questo caso la n-pla di premesse viene comunque considerata come se fosse in congiunzione ($\land$).
...
Nell'esempio precedente $s_2$ è un teorema del sistema formale ($s_1 \land (s_1 \Rightarrow s_2) \vdash s_2$).
...
Rispetto l'esempio precedente l'insieme $\{s_1, s_1 \Rightarrow s_2\}$ è una teoria.
In notazione si scrive $\vdash_T \varphi$ per indicare che la formula $\varphi$ è dimostrabile nella teoria $T$.


Tornando alla tesi di Gödel si basa sulla costruzione di una codifica delle formule e delle dimostrazioni
